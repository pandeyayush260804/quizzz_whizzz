...
const startServer = async () => {
  try {
    await ensureDb(); // Connect DB
    app.listen(PORT, () => {
      console.log(
        chalk.greenBright.bold(`🚀 QuizWhizz Server running at http://localhost:${PORT}`)
      );
    });
  } catch (err) {
    console.error(chalk.redBright.bold('❌ Server startup failed'), err);
  }
};
